 ...
 * GLOBAL HOOKS
 ********************/
// Smooth custom cursor with trail and interactive scaling
function useCustomCursor() {
  const [pos, setPos] = useState({ x: -100, y: -100 })
  const [trail, setTrail] = useState([]) // last 4 positions
  ...
  useEffect(() => {
    ...
    const onOver = (e) => {
      const el = e.target.closest('a, button, input, textarea, select, [data-interactive]')
      setInteractive(!!el)
    }
...
function TopProgressBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[9999] h-[2px] bg-transparent">
      <div className="h-full bg-[linear-gradient(90deg,#2563EB,#7C3AED,#DB2777)]" style={{ width: `${Math.min(100, Math.max(0, progress * 100))}%` }} />
    </div>
  )
}
...
function Navbar() {
  ...
  return (
    <div className={`fixed top-0 left-0 right-0 z-[1000] transition-colors ${scrolled ? 'backdrop-blur-lg bg-[rgba(15,23,42,0.75)] border-b border-white/10' : 'bg-transparent'}`}>
      <div className="max-w-[1200px] mx-auto h-20 px-6 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <a data-interactive href="#pricing" className="px-4 py-2 rounded-lg font-semibold bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow-[0_10px_30px_rgba(99,102,241,0.35)]">Começar Grátis</a>
        </div>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'tween', duration: 0.25 }} className="fixed inset-0 bg-[rgba(10,10,20,0.9)] backdrop-blur-xl md:hidden">
            ...
            <div className="h-full w-full flex items-center justify-center">
              <div className="flex flex-col items-center gap-6 text-xl">
                <a onClick={()=>setOpen(false)} href="#features" className="text-white/80 hover:text-white">Features</a>
                <a onClick={()=>setOpen(false)} href="#how-it-works" className="text-white/80 hover:text-white">Como Funciona</a>
                <a onClick={()=>setOpen(false)} href="#pricing" className="text-white/80 hover:text-white">Preços</a>
                <a onClick={()=>setOpen(false)} href="#faq" className="text-white/80 hover:text-white">FAQ</a>
...
function LoadingOverlay({ onDone }) {
  ...
  return (
    <AnimatePresence>
      {visible && (
        <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.5 }} className="fixed inset-0 z-[2000] grid place-items-center bg-[linear-gradient(135deg,#0b1020,#0c0720_40%,#130d2e)]">
          <div className="text-center">
            ...
            <div className="w-64 h-2 bg-white/10 rounded-full overflow-hidden mx-auto">
              <motion.div className="h-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500" initial={{ width: '0%' }} animate={{ width: '100%' }} transition={{ duration: 1.2 }} />
            </div>
...
function GlowBadge() {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-blue-500/30 bg-blue-500/10 text-blue-200 backdrop-blur-md shadow-[0_0_30px_rgba(59,130,246,0.25)]">
...
function GradientHeadline() {
  return (
    <motion.h1 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-4xl md:text-6xl lg:text-7xl font-extrabold leading-tight tracking-tight bg-clip-text text-transparent bg-[linear-gradient(135deg,#60A5FA,#A78BFA,#EC4899)]">
...
function Subheadline() {
  return (
    <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-base md:text-lg text-white/70 max-w-xl mt-6">
...
function CTAs() {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col sm:flex-row items-center gap-4 mt-8">
      <a data-interactive href="#features" className="px-8 py-4 rounded-xl text-lg font-semibold bg-gradient-to-r from-blue-500 to-purple-500 hover:shadow-[0_10px_40px_rgba(99,102,241,0.5)] hover:scale-[1.03] transition-transform shadow-blue-500/30">
        Começar Grátis →
      </a>
...
function SocialProof() {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="mt-8 flex items-center gap-3 text-sm text-white/70">
...
function Logo() {
  // Easter egg: click 5x rotates & particles
  const [clicks, setClicks] = useState(0)
  const [boom, setBoom] = useState(false)
  useEffect(() => {
    if (clicks >= 5) {
      setBoom(true)
      const id = setTimeout(() => { setBoom(false); setClicks(0) }, 1200)
      return () => clearTimeout(id)
    }
  }, [clicks])
  return (
...
function Hero3D() {
  ...
  const translate = useMemo(() => `translateY(${-(offset * 0.05)}px)`, [offset])
  return (
...
function HolographicCodeCard() {
  ...
  return (
    <motion.pre whileHover={{ rotateX: 6, rotateY: -6 }} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] max-w-xl p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/20 text-white shadow-[0_0_80px_rgba(168,85,247,0.35)]" style={{ perspective: 1000 }}>
...
function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col lg:flex-row items-center pt-20">{/* padding for navbar */}
...
function ScrollIndicator() {
  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 text-white/60 text-xs flex flex-col items-center gap-2">
      <span>scroll</span>
      <motion.div className="w-[2px] h-8 bg-white/30 rounded" animate={{ y: [0, 6, 0], opacity: [0.6, 1, 0.6] }} transition={{ repeat: Infinity, duration: 1.8 }} />
    </div>
  )
}
...
function StepNumber({ n }) {
  return (
    <div className="relative select-none">
      <span className="block text-5xl md:text-7xl font-extrabold text-transparent bg-clip-text bg-[linear-gradient(135deg,#60A5FA,#A78BFA)]" style={{ WebkitTextStroke: '2px rgba(167,139,250,0.6)' }}>{n}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="max-w-[320px] md:max-w-[420px] rounded-2xl px-4 py-3 bg-white/10 backdrop-blur-xl border border-white/15 shadow-[0_10px_40px_rgba(99,102,241,0.25)]">
        <p className="text-white/90 text-sm md:text-base">{content}<span className="animate-pulse">▋</span></p>
      </div>
    </div>
  )
}
      ...
      <div className="font-mono text-xs md:text-sm text-white/80 space-y-1">
        {Array.from({ length: lines }).map((_, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <span className="flex-1"><span className="text-sky-300">const</span> <span className="text-pink-300">comp</span> = <span className="text-emerald-300">() =&gt;</span> <span className="text-white">{'{'}</span>{i < (tick % (lines + 1)) ? <span className="text-white/70"> ...</span> : <span className="opacity-30"> //</span>}</span>
          </div>
        ))}
      </div>
    ...
    <div className="relative w-full rounded-xl overflow-hidden border border-white/10 bg-slate-950/60">
      <div className="grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="relative min-h-[140px] md:min-h-[180px] bg-gradient-to-br from-slate-800/40 to-slate-900/60">
          <motion.div className="absolute w-6 h-6 rounded-full bg-white/90 text-slate-800 flex items-center justify-center text-xs" animate={{ x: [20, 120, 60, 160], y: [20, 30, 80, 40] }} transition={{ repeat: Infinity, duration: 4 }}>⌁</motion.div>
...
function SuccessVisual() {
  return (
    <div className="relative w-full rounded-xl border border-white/10 bg-slate-950/60 p-6 overflow-hidden">
      <motion.div className="mx-auto w-16 h-16 rounded-full bg-gradient-to-br from-emerald-400 to-green-600 flex items-center justify-center text-3xl shadow-[0_0_40px_rgba(16,185,129,0.5)]" animate={{ scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, duration: 2 }}>✓</motion.div>
      ...
      {Array.from({ length: 18 }).map((_, i) => (<motion.span key={i} className="absolute w-2 h-2 rounded-sm" style={{ left: `${(i * 11) % 100}%`, top: -10, background: i % 2 ? '#60A5FA' : '#A78BFA' }} animate={{ y: [0, 180], rotate: [0, 180] }} transition={{ repeat: Infinity, duration: 2 + (i % 5) * 0.3, delay: i * 0.1 }} />))}
    </div>
  )
}
...
function Connector({ progress }) {
  return (
    <div className="absolute left-6 md:left-1/2 md:-translate-x-1/2 top-0 h-full w-1 md:w-[3px]">
      <div className="relative h-full">
        <div className="absolute left-1/2 -translate-x-1/2 w-[3px] h-full bg-white/5 rounded" />
        <motion.div className="absolute left-1/2 -translate-x-1/2 w-[3px] bg-gradient-to-b from-blue-500 to-purple-500 rounded" style={{ height: `${progress * 100}%` }} />
      </div>
    </div>
  )
}
...
function Step({ index, side, visual, title, description }) {
  ...
  return (
    <div ref={ref} className="relative grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
      <motion.div initial={{ opacity: 0, x: isLeft ? -40 : 40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className={`order-1 ${isLeft ? '' : 'md:order-2'}`}>{visual}</motion.div>
      <motion.div initial={{ opacity: 0, x: isLeft ? 40 : -40 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.05 }} className={`order-2 ${isLeft ? '' : 'md:order-1'}`}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <div className="relative">
          ...
          <div className="space-y-24">
            ...
            <Step index="02" side="right" visual={<CodeWriter />} title="IA Gera Código Profissional" description="Claude AI analisa seu pedido e escreve React + TypeScript otimizado. Componentes reutilizáveis, código limpo." />
            <Step index="03" side="left" visual={<SplitPreview />} title="Visualize e Ajuste em Tempo Real" description="Cada mudança reflete instantaneamente. Não gostou? Só pedir ajustes. 'Mude a cor para azul', 'Adicione um footer'." />
            <Step index="04" side="right" visual={<SuccessVisual />} title="Exporte ou Publique" description="Baixe todo o código ou publique com um clique. Domínio gratuito incluído. Online em 30 segundos." />
          </div>
        </div>
        <div className="mt-16 flex justify-center">
          <a data-interactive href="#pricing" className="group inline-flex items-center gap-3 px-8 py-5 rounded-2xl text-[20px] font-semibold bg-[linear-gradient(90deg,#2563EB,#7C3AED,#DB2777)] text-white shadow-[0_0_40px_rgba(124,58,237,0.5)] hover:shadow-[0_0_60px_rgba(124,58,237,0.7)] transition-transform hover:scale-[1.02]">Experimentar Agora - Grátis <span className="text-2xl group-hover:translate-x-1 transition-transform">🚀</span></a>
        </div>
      </div>
    </section>
  )
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        ...
        <div className="relative flex justify-center">
          {Array.from({ length: 16 }).map((_, i) => (
            <motion.span key={i} className="pointer-events-none absolute w-2 h-2 rounded-full" style={{ background: i % 2 ? '#60A5FA' : '#A78BFA', left: `${(i * 7) % 100}%`, top: `${(i * 13) % 100}%`, filter: 'blur(0.5px)' }} animate={{ y: [0, -10, 0], opacity: [0.6, 1, 0.6] }} transition={{ repeat: Infinity, duration: 3 + (i % 5) * 0.3, delay: i * 0.1 }} />
          ))}
          <motion.div ref={cardRef} onMouseMove={onMove} onMouseLeave={onLeave} style={{ transform: `perspective(1000px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg)` }} className="relative w-full max-w-[500px] rounded-[32px] p-[2px] bg-[conic-gradient(from_0deg,rgba(99,102,241,0.8),rgba(168,85,247,0.8),rgba(219,39,119,0.8),rgba(99,102,241,0.8))] animate-[spin_8s_linear_infinite]">
            <div className="rounded-[30px] p-12 bg-slate-900/70 backdrop-blur-xl border border-white/10 shadow-[0_0_80px_rgba(99,102,241,0.35),0_0_120px_rgba(168,85,247,0.25)] relative overflow-hidden">
              ...
              <div className="mb-6 flex justify-center"><span className="px-4 py-1.5 rounded-full text-xs font-semibold text-white bg-gradient-to-r from-emerald-500 to-green-600 shadow-[0_0_20px_rgba(16,185,129,0.4)]">GRÁTIS PARA SEMPRE</span></div>
              ...
              <a data-interactive href="#" className="mt-10 block w-full h-14 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-bold text-[18px] grid place-items-center shadow-[0_10px_40px_rgba(99,102,241,0.4)] hover:shadow-[0_10px_60px_rgba(99,102,241,0.6)] transition-transform hover:scale-[1.02]">Criar Conta Gratuita →</a>
...
function TestimonialsSection() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      if (!paused) setOffset((o) => (o - 1))
    }, 16)
    return () => clearInterval(id)
  }, [paused])

  const cards = [...testimonials, ...testimonials] // loop

  return (
    <section id="testimonials" className="relative py-24 md:py-28">
      <div className="absolute inset-0 bg-[radial-gradient(800px_400px_at_20%_10%,rgba(99,102,241,0.12),transparent),radial-gradient(900px_500px_at_80%_70%,rgba(14,165,233,0.1),transparent)]" />
      <div className="relative z-10 max-w-6xl mx-auto px-6">
          ...
          <div className="flex gap-6" style={{ transform: `translateX(${offset}px)` }}>
            {cards.map((t, i) => (
              <div key={i} className="min-w-[280px] md:min-w-[360px] bg-white/10 backdrop-blur-xl rounded-2xl p-8 border border-white/10 relative">
                <div className="absolute inset-0 rounded-2xl pointer-events-none" style={{ border: '1px solid', borderImage: 'linear-gradient(120deg,rgba(96,165,250,0.3),rgba(167,139,250,0.3)) 1', }} />
...
function FAQItem({ q, a }) {
  ...
  return (
    <div className="border-b border-white/10">
      ...
      <motion.div initial={false} animate={{ height }} className="overflow-hidden">
        <div ref={contentRef} className="pb-5 px-2 text-white/70">{a}</div>
      </motion.div>
    </div>
  )
}
...
function FinalCTA() {
  return (
    <section className="relative py-[120px] bg-[linear-gradient(135deg,#0f1c3a,#3b82f6_30%,#7c3aed_80%)]">
      <div className="absolute inset-0 opacity-20 bg-[radial-gradient(800px_400px_at_50%_0%,white,transparent)]" />
      <div className="relative z-10 max-w-4xl mx-auto px-6 text-center">
        ...
        <a data-interactive href="#pricing" className="mt-8 inline-block px-8 py-4 rounded-2xl bg-white text-slate-900 font-bold text-lg md:text-xl shadow-[0_20px_60px_rgba(255,255,255,0.2)] hover:scale-[1.02] transition">Começar Gratuitamente</a>
      </div>
    </section>
  )
}
      ...
      <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div>
          ...
          <ul className="space-y-2">
            {['Features','Pricing','Templates','Documentação','Changelog','Roadmap'].map((t,i)=>(<li key={i}><a data-interactive href="#" className="text-white/70 hover:text-white hover:underline transition">{t}</a></li>))}
          </ul>
        </div>
        <div>
          <div className="text-white font-semibold mb-3">Recursos</div>
          <ul className="space-y-2">
            {['Blog','Tutoriais','Comunidade','Suporte','Status'].map((t,i)=>(<li key={i}><a data-interactive href="#" className="text-white/70 hover:text-white hover:underline transition">{t}</a></li>))}
          </ul>
        </div>
        <div>
          <div className="text-white font-semibold mb-3">Legal</div>
          <ul className="space-y-2">
            {['Termos de Uso','Privacidade','Cookies'].map((t,i)=>(<li key={i}><a data-interactive href="#" className="text-white/70 hover:text-white hover:underline transition">{t}</a></li>))}
          </ul>
        </div>
      </div>
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (konami.every((k, i) => seq[i] === k)) {
        alert('🎉 Easter egg encontrado! Você é curioso hein?')
      }
    }
    let devSeq = []
    let last = 0
    const onKeyDev = (e) => {
      const now = Date.now()
      if (now - last > 800) devSeq = []
      last = now
      devSeq.push(e.key.toLowerCase())
      if (devSeq.slice(-3).join('') === 'dev') {
        console.log('👨‍💻 Olá developer! Gostou do código? Estamos contratando!')
      }
    }
...
function CustomCursorLayer() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[3000]">
      ...
      <motion.span className="absolute rounded-full" style={{ left: pos.x - 8, top: pos.y - 8, width: 16, height: 16, background: 'linear-gradient(135deg,#60A5FA,#A78BFA)', opacity: 0.85 }} animate={{ scale: interactive ? 2 : 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }} />
    </div>
  )
}
